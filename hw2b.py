...
def findEvents(symbols, startday,endday, marketSymbol,verbose=False):
	...
	for symbol in symbols:
	    for i in range(1,len(mktneutDM[symbol])):
	        if SPYValues[i] < -0.03 and mktneutDM[symbol][i] > -0.01 : # When the stock drop below 5 for the first time
...
eventProfiler.study(filename="hw2b.pdf",plotErrorBars=True,plotMarketNeutral=True,plotEvents=False,marketSymbol='SPY')
