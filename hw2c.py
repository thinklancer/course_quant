...
#	close = (close.fillna(method='ffill')).fillna(method='backfill')
	...
	for symbol in symbols:
	    ...
	    for i in range(2,len(close[symbol])):
	        if close[symbol][i-1] >= 10. and close[symbol][i] < 10. : # When the stock drop below 5 for the first time
...
eventProfiler.study(filename="hw2c.pdf",plotErrorBars=True,plotMarketNeutral=True,plotEvents=False,marketSymbol='SPY')
